...
def login_post():
    ...
    account_locked_response = requests.get(f"http://localhost:3307/checkAccountLocked/{username}")
    ...
    # Invalid credentials
        ...
        # Increase login attempts only if it was an invalid credential response
        if 'loginAttempts' not in session:
            session['loginAttempts'] = 0
        ...
        if session['loginAttempts'] >= 3:
            ...
            url = "http://localhost:3307/accountLocked"
            data = {"username": username}
            headers = {"Content-Type": "application/json"}
...
def medical_ward():
    return render_template('medical_ward.html', username=session['username'], role=session['role']) 

@app.route('/radiology')
def radiology():
    return render_template('radiology.html', username=session['username'], role=session['role']) 
...
def invoice_management():
    conn = fmsdb  # Use your existing database connection
    cursor = conn.cursor(dictionary=True)
    cursor.execute("SELECT * FROM invoices")  # Adjust your query based on the actual table and columns
    invoices_list = cursor.fetchall()  # Fetch all invoice records
    cursor.close()
    
    # Pass the invoice data to your template
    return render_template('invoice_management.html', username=session['username'], role=session['role'], invoices=invoices_list)

@app.route('/filter_invoices')
def filter_invoices():
    # Get filter values from request arguments
    status = request.args.get('status')
    approval_status = request.args.get('approval_status')

    conn = fmsdb  # Your existing database connection
    cursor = conn.cursor(dictionary=True)

    # Base query to get invoices
    query = "SELECT * FROM invoices WHERE 1=1"  # The 1=1 allows us to append conditions easily
    conditions = []

    # Append conditions based on filters
    if status and status != '':
        conditions.append(f"payment_status = '{status}'")
    
    if approval_status and approval_status != '':
        conditions.append(f"approval_status = '{approval_status}'")

    # If there are conditions, join them into the query
    if conditions:
        query += " AND " + " AND ".join(conditions)

    cursor.execute(query)  # Execute the query with filters
    invoices = cursor.fetchall()  # Fetch the filtered invoices
    cursor.close()

    # Return the filtered invoice rows
    return render_template('invoice_rows.html', invoices=invoices)
...
@app.route('/sub-view')
def sub_view():
    return render_template('submitted_view.html', username=session['username'], role=session['role'])    

@app.route('/invoice_edit')
def invoice_edit():
    return render_template('invoice_edit.html', username=session['username'], role=session['role'])  

@app.route('/invoice_coverage')
def invoice_coverage():
    return render_template('invoice_coverage.html', username=session['username'], role=session['role'])  

@app.route('/invoice_reimbursement')
def invoice_reimbursement():
    return render_template('invoice_reimbursement.html', username=session['username'], role=session['role']) 

@app.route('/emp_edit')
def emp_edit():
    return render_template('employee_edit.html', username=session['username'], role=session['role']) 
